            ...
            <div className="hidden md:grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 mt-6">
                {services.map((service) => (
                    <Button
                        key={service}
                        variant="default"
                        className="h-12 px-6 bg-[#3a4ded]/30 rounded-lg text-center text-white cursor-pointer hover:shadow-lg hover:scale-105 active:scale-95 transition-all duration-150 w-full"
                    >
                        {service}
                    </Button>
                ))}
